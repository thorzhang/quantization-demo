...
# 2. 因子计算
# =========================================================
def calculate_factor_library(df):
...
# =========================================================
def cross_sectional_normalize(df, column):
...
def calculate_dynamic_scores(df):
    """计算动态因子得分"""
    df = df.copy()
    ...
    df = cross_sectional_normalize(df.assign(momentum_raw=momentum_raw), 'momentum_raw')
    ...
    df = cross_sectional_normalize(df.assign(quality_raw=quality_raw), 'quality_raw')
    ...
    df = cross_sectional_normalize(df.assign(technical_raw=technical_raw), 'technical_raw')
...
# =========================================================
# 4. 市场择时（修正：移除多余的shift）
# =========================================================
def calculate_market_timing(df, lookback_days=60, min_trading_days_ratio=0.2):
    """
...
# 5. 信号生成
# =========================================================
def generate_signals(df, max_positions=8, min_score=0.5):
    """
    信号生成：T日收盘后生成信号
    信号将用于T+1日开盘买入
    """
    ...
    #         mask = df['date'] == date
    #         n_signals = min(max_positions, mask.sum())
    #         if n_signals > 0:
    #             top_codes = df.loc[mask].nlargest(n_signals, 'score')['code'].values
    #             df.loc[mask & df['code'].isin(top_codes), 'signal'] = True
...
# 6. 回测引擎（时序正确）
# =========================================================
def backtest(df, initial_capital=1000000, transaction_cost=0.0005,
    ...
    for date in trading_days:
        ...
        if len(day_data) > 0:
            daily_data[date] = {
                'open': day_data.set_index('code')['open'].to_dict(),
                'close': day_data.set_index('code')['close'].to_dict(),
                'high': day_data.set_index('code')['high'].to_dict(),
                'low': day_data.set_index('code')['low'].to_dict(),
                'signals': day_data[day_data['signal']]['code'].tolist(),  # T日信号
                'scores': day_data.set_index('code')['score'].to_dict()
            }
    ...
    for i, today in enumerate(trading_days):
        ...
        for code, pos in positions.items():
            if code not in today_data.get('high', {}):
                continue

            buy_price = pos['buy_price']
            high_ret = (today_data['high'][code] - buy_price) / buy_price
            low_ret = (today_data['low'][code] - buy_price) / buy_price

            if low_ret <= stop_loss:
                sell_price = today_data['close'][code]  # 止损用收盘价
            ...
            if yesterday in daily_data:
                ...
                if len(yesterday_signals) > 0:
                    ...
                    for code in list(positions.keys()):
                        if code not in new_signal_set and code in today_data['open']:
                            pos = positions[code]
                            sell_price = today_data['open'][code]
                            sell_value = pos['shares'] * sell_price
                            cash += sell_value * (1 - transaction_cost)
                            trades.append({
                                'code': code,
                    ...
                    # 买入新信号（使用今日开盘价）
                    buy_codes = [c for c in yesterday_signals[:max_positions] if c not in positions]
                    if len(buy_codes) > 0 and cash > 10000:
                        per_stock = cash * 0.9 / len(buy_codes)
                        for code in buy_codes:
                            if code in today_data['open']:
                                buy_price = today_data['open'][code]
        ...
        pos_value = 0
        for code, pos in positions.items():
            if code in today_data['close']:
                pos_value += pos['shares'] * today_data['close'][code]
            else:
